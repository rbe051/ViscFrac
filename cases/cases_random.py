"""
Module for setting up and running the cases with random fracture networks
from Paper:
...
from cases import random_problem
...
def run_K_A_variation(run_id):
    ...
    folder = os.path.join("results", "res_random_fracs", "KA")
    return run_random_frac(Pe, R, K, A, N, file_name, folder, mesh_size=320, physdims=[4, 1])
...
def run_Pe_R_variation(run_id):
    ...
    N = 1
    file_name = "Pe_{}_R_{}_K_{}_A_{}_N_{}_task_{}".format(Pe, R, K, A, N, run_id)
    folder = os.path.join("results", "res_random_fracs", "PeR")
    return run_random_frac(Pe, R, K, A, N, file_name, folder)


def run_N_variation(run_id):
    np.random.seed()
    num_fracs = [1, 2, 3, 4, 5]
    mesh_sizes = [320, 320, 320, 320, 320]
    R = 2
    Pe = 500
    K = 10
    A = 1e-3
    N = num_fracs[run_id]
    file_name = "Pe_{}_R_{}_K_{}_A_{}_N_{}_task_{}".format(Pe, R, K, A, N, run_id)
    folder = os.path.join("results", "res_random_fracs", "N")
    physdims= [4, 1]

    return run_random_frac(Pe, R, K, A, N, file_name, folder, 200, mesh_sizes[run_id], physdims)
...
def run_F_variation(run_id):
    ...
    N = 1
    file_name = "Pe_{}_R_{}_K_{}_A_{}_N_{}_task_{}".format(Pe, R, K, A, N, run_id)
    folder = os.path.join("results", "res_random_fracs", "F")
    return run_random_frac(Pe, R, K, A, N, file_name, folder)


def run_F_variation_repeated(run_id):
    np.random.seed()
    As = [
...
def run_random_frac(Pe, R, K, A, N, file_name, folder, end_time=100, mesh_size=None, physdims=None, local_grid_adaptation=False):
    if local_grid_adaptation:
        raise ValueError("Local grid refinement not supported by random fractures")
    else:
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) != 3:
        print("Invalid argument list. Usage: random_fractures.py <case_id> <run_id>")
        raise ValueError()
